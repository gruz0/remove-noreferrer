...
curl -XGET $URL --silent | grep page_link | grep noreferrer > /dev/null
...
docker-compose $COMPOSER_ARGS exec wordpress wp option add remove_noreferrer '{"where_should_the_plugin_work":["page"]}' --format=json --allow-root > /dev/null
